...
type authScreenProp = StackNavigationProp<RootStackParamList, 'Tomorrow'>;

const Tomorrow = ({weather, temperature} : {weather:any, temperature:any}) => { 
    ...
    const getWeather = async (lat: string, long: string) => {
        try {
            ...
            var weatherTemp = json['daily'][1]['weather'][0]['description'];
            ...
            let iconRq = getIcon(json['daily'][1]);
        
            // console.error(Math.round(json['current']['temp']).toString());
            setTemp(Math.round(json['daily'][1]['temp']['day']).toString());
            setWeatherText(weatherTemp);
            setIcon(iconRq);
            setMin(Math.round(json['daily'][1]['temp']['min']).toString());
            setMax(Math.round(json['daily'][1]['temp']['max']).toString());
            setList(tempHourly);
